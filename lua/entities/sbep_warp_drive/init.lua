...
function ENT:SpawnFunction( ply, tr )
	local ent = ents.Create("sbep_warp_drive") 		-- Create the entity
...
function ENT:TriggerInput(iname, value)
	...
	--[[	print( timer.IsTimer( "warpdrivewaittime" ) ) ]]
		if (CurTime()-self.NTime)>7 and !timer.Exists( "warpdrivewaittime" ) and self.JumpCoords.Dest~=self.Entity:GetPos() and util.IsInWorld(self.JumpCoords.Dest) then
			...
			timer.Create( "warpdrivewaittime", 1, 1, function() self.Entity:Go() timer.Remove("warpdrivewaittime") end)
		else
...
function ENT:Go()
	...
	if(self.Constrained == 1) then
		...
		for _, v in pairs(self.ConstrainedEnts) do
			if v:IsValid() and !self.DoneList[v] and (v:CPPIGetOwner() == self:CPPIGetOwner() or v:IsPlayer()) then
