...
if (CLIENT) then
    ...
    language.Add("Tool.sbep_smb_skin_selector.0", "Left click a prop to switch to the selected skin. Right click to apply to all constrained.")
end
...
function TOOL:RightClick(trace)
    ...
    if trace.Entity:IsValid() then
        ...
        for k, v in pairs(constraint.GetAllConstrainedEntities(trace.Entity)) do
            if string.find(string.lower(v:GetModel()), "smallbridge") then

                local SkinInt = 0
                if v:SkinCount() == 10 then
                    SkinInt = 1
                else
                    SkinInt = 0
                end
